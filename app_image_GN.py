...
model = joblib.load('models/image_model_googlenet.pkl')

# Preprocess image
def preprocess_image(image_path):
    image = cv2.imread(image_path) # Read in image
    image_resized = cv2.resize(image, (150 , 150)) # Resize
    image_resized_rgb =  cv2.cvtColor(image_resized, cv2.COLOR_BGR2RGB) # Convert colour from BGR (cv2's default) to RGB
    image_resized_rgb_inception = preprocess_input(image_resized_rgb) # Preprocess image for Inception
    
    # Current shape: (150, 150, 3)
    ...
    preprocessed_image = np.expand_dims(image_resized_rgb_inception, axis=0)

    # Current shape: (1, 150, 150, 3)
    
    return preprocessed_image

@app.route('/', methods=['GET'])
def index():
    return render_template('index_image_GN.html')
...
def upload_file():
    ...
    if file and file.filename.endswith('.jpg'):
        ...
        return render_template('index_image_GN.html', result=result)
